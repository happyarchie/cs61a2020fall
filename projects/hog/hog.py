...
def roll_dice(num_rolls, dice=six_sided):
    ...
    #     if o == 1:
    #         return 1
    #     sum, index = sum + o, index + 1
    #     print("DEBUG: dice() is ", o)
    # return sum

    # remember that in this code if we use if-return, we will miss the times
    # behind the number 1, like roll_dice(3,make_test_dice(4, 1, 2, 6)), if we
    # use if-return here, we will missed the third times(2), and next time we call
    # this expression, 2 will be used and we get a wrong results.
    ...
    # END PROBLEM 1
    # in this correct code we use flag to tackle the problme in the PSEUDOCODE,
    # as this way we can calculate all the roll_nums and won't missed any times.
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            player0_score = take_turn(num_rolls, score1, dice)
            score0 = score0 + player0_score
            say = say(score0, score1)
            # if score0 > goal:
            #     break
            if not extra_turn(score0, score1): # Only change 'who' when there is no extra turn
                who = other(who)

        elif who == 1:  #在who变为1后，如果使用if将直接执行下列代码，导致结果出现问题，而elif将开启一个新的循环后执行，
            num_rolls = strategy1(score1, score0)
            player1_score = take_turn(num_rolls, score0, dice)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def get_average(*args):
        time, result = 0, 0
        while time < trials_count: 
            result, time = result + original_function(*args), time +1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+free_bacon(opponent_score), opponent_score):
        ...
        result = bacon_strategy(score, opponent_score, cutoff, num_rolls)
        return result
